...
public class GeometryHelper
{
  ...
  {
    GeoPoint closestPoint = getClosestPointOnSegment(sx1, sy1, sx2, sy2, px, py);
    return getDistance(closestPoint.getLatitude(), closestPoint.getLongitude(), px, py);
  }
   ...
   */
  private static GeoPoint getClosestPointOnSegment(int sx1, int sy1, int sx2, int sy2, int px, int py)
  {
    ...
    if (u < 0)
    {
      closestPoint = new GeoPoint(sx1, sy1);
    }
    else if (u > 1)
    {
      closestPoint = new GeoPoint(sx2, sy2);
    }
    else
    {
      closestPoint = new GeoPoint(Math.round(sx1 + u * xDelta), Math.round(sy1 + u * yDelta));
    }
